...
export const POST = route(async req => {
  if (!(await currentFacilitator(req))) return json({ error: 'unauthorised' }, 401);
  if (!process.env.WHISPER_URL) return json({ error: 'transcription is not configured' }, 501);

  const form = await req.formData();
  const file = form.get('audio');
  const langHint = form.get('lang') || undefined;
  if (!file) return json({ error: 'audio required' }, 400);
  ...
  if (!r.ok) return json({ error: 'transcription failed', detail: await r.text() }, 502);
  ...
  return json({ text, language, segments });
});
